...
describe('LanguageCvService', () => {
  ...
  beforeEach(async () => {
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        LanguageCvService,
        { provide: getRepositoryToken(LanguageCv), useValue: mockRepo },
  ...
  it('addToCv: dodaje language do CV', async () => {
    mockCvService.findOne.mockResolvedValue({ id: 1 });
    const entity = { id: 10, cv: { id: 1 }, language: { id: 2 } };
    mockRepo.create.mockReturnValue(entity);
    mockRepo.save.mockResolvedValue(entity);
    const result = await service.addToCv(1, { languageId: 2 }, 5);
  ...
  it('addToCv: propaguje NotFoundException gdy CV nie istnieje', async () => {
    ...
    await expect(service.addToCv(99, { languageId: 1 }, 5)).rejects.toThrow(NotFoundException);
  });
