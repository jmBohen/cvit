...
describe('TechnologyCvService', () => {
  ...
  beforeEach(async () => {
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        TechnologyCvService,
        { provide: getRepositoryToken(TechnologyCv), useValue: mockRepo },
  ...
  it('addToCv: dodaje technology do CV', async () => {
    mockCvService.findOne.mockResolvedValue({ id: 1 });
    const entity = { id: 10, cv: { id: 1 }, technology: { id: 2 } };
    mockRepo.create.mockReturnValue(entity);
    mockRepo.save.mockResolvedValue(entity);
    const result = await service.addToCv(1, { technologyId: 2 }, 5);
  ...
  it('addToCv: propaguje NotFoundException gdy CV nie istnieje', async () => {
    ...
    await expect(service.addToCv(99, { technologyId: 1 }, 5)).rejects.toThrow(NotFoundException);
  });
