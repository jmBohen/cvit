...
describe('EducationCvService', () => {
  ...
  beforeEach(async () => {
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        EducationCvService,
        { provide: getRepositoryToken(EducationCv), useValue: mockRepo },
  ...
  it('addToCv: dodaje education do CV', async () => {
    mockCvService.findOne.mockResolvedValue({ id: 1 });
    const entity = { id: 10, cv: { id: 1 }, education: { id: 2 } };
    mockRepo.create.mockReturnValue(entity);
    mockRepo.save.mockResolvedValue(entity);
    const result = await service.addToCv(1, { educationId: 2 }, 5);
  ...
  it('addToCv: propaguje NotFoundException gdy CV nie istnieje', async () => {
    ...
    await expect(service.addToCv(99, { educationId: 1 }, 5)).rejects.toThrow(NotFoundException);
  });
