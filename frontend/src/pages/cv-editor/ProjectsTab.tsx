...
export default function ProjectsTab({ cvId }: { cvId: number }) {
  ...
  const toggleMutation = useOptimisticToggle(cvId, 'project', 'projects', 'cv-projects');
                ...
                <label className="flex items-start cursor-pointer flex-1">
                  <div className="flex items-center h-5 mt-1">
                    <input
                      type="checkbox"
                      checked={inCv}
                      onChange={() => toggleMutation.mutate({ id: proj.id, inCv })}
        ...
        <ul className="space-y-4">
          ...
          {cvProjects?.map((item) => (
            <li key={item.id} className="bg-white p-4 rounded-md shadow-sm border border-slate-200 flex justify-between items-start group">
              ...
              <button 
                onClick={() => toggleMutation.mutate({ id: item.proj.id, inCv: true })}
